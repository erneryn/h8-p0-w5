...
for(var i=0;i<list.length;i++){
   tampung.push(list[i].split(","))
}
for(var j=0;j<tampung.length;j++){
    if(Number(tampung[j][2]) > 19 ){
        if(tampung[j][1]== "male"){
            result.push("Mr."+tampung[j][0])
        } else if(tampung[j][1] == "female" && Number(tampung[j][2])>30){
            result.push("Mrs."+tampung[j][0])
        } else if (tampung[j][1] == "female" && Number(tampung[j][2]<30)){
            result.push("Ms."+tampung[j][0])
        }
